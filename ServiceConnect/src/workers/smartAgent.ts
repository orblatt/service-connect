...
export const emailDigest: SmartAgentJob<emailDigestInput, any> = async (
    ...
    context: any) => {
        ...
        searchProfiles.forEach(async (searchProfile: SearchProfile) => {
            const { minPrice, maxPrice, isDone, category, emails, city, minDuration, maxDuration, exactDuration } = searchProfile;
            const jobAdFilters: JobAdFilters = { minPrice, maxPrice, isDone, category, city, minDuration, maxDuration, exactDuration}; 
